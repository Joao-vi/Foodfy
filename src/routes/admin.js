...
const { onlyUsers, onlyUsersAdmin, adminOrCreatorUser } = require('../app/middlewares/session')
...
routes.get("/recipes/:id/edit", adminOrCreatorUser, recipesAdmin.edit); // Mostrar formulário de edição de receita

routes.post("/recipes", onlyUsers, multer.array("photos", 5), recipesAdmin.post); // Cadastrar nova receita
routes.put("/recipes", multer.array("photos", 5), adminOrCreatorUser, recipesAdmin.put); // Editar uma receita
routes.delete("/recipes", adminOrCreatorUser, recipesAdmin.delete); // Deletar uma receita
